...
class _EditProfileState extends State<EditProfile> {
  ...
  final String imgBBKey = "PASTE_YOUR_IMGBB_API_KEY_HERE"; 
  ...
  void fetchUserData() async {
    if (user != null) {
      DocumentSnapshot userData = await FirebaseFirestore.instance.collection('Users').doc(user!.email).get();
      if (userData.exists) {
        setState(() {
  ...
  Future<void> pickAndUploadImage() async {
    ...
    if (pickedFile != null) {
      CroppedFile? croppedFile = await ImageCropper().cropImage(
        ...
        uiSettings: [
          AndroidUiSettings(toolbarTitle: 'Adjust Photo', toolbarColor: Colors.brown, toolbarWidgetColor: Colors.white, initAspectRatio: CropAspectRatioPreset.square, lockAspectRatio: true),
        ],
      );
      ...
      if (croppedFile != null) {
        ...
        try {
          ...
          if (response.statusCode == 200) {
            ...
            await FirebaseFirestore.instance.collection('Users').doc(user!.email).update({'profileImage': newUrl});
  ...
  void updateProfile() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('Users').doc(user!.email).update({'name': nameController.text.trim(), 'phone': phoneController.text.trim()});
      if (passwordController.text.isNotEmpty) {
        if (passwordController.text == confirmPasswordController.text) {
          await user!.updatePassword(passwordController.text);
          await FirebaseFirestore.instance.collection('Users').doc(user!.email).update({'password': passwordController.text});
        }
      }
        ...
        child: Column(
          children: [
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        ...
                        Row(children: [IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, color: Colors.white, size: 20), onPressed: () => Navigator.pop(context)), const Expanded(child: Text("Edit Profile", textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)))]),
                                ...
                                child: CircleAvatar(
                                  ...
                                  child: isUploading 
                                      ...
                                      : profileImageUrl != null 
                                          ...
                                          : const Icon(Icons.person_rounded, size: 65, color: Colors.brown),
                                ),
                              ),
            ...
            Padding(
              ...
              child: Column(
                children: [
                  buildInputField(Icons.person_outline_rounded, "Full Name", nameController),
                  buildInputField(Icons.phone_android_rounded, "Phone Number", phoneController),
                  buildPasswordField("New Password", passwordController),
                  buildPasswordField("Confirm Password", confirmPasswordController),
                  const SizedBox(height: 30),
                  SizedBox(width: double.infinity, height: 55, child: ElevatedButton(onPressed: loading ? null : updateProfile, style: ElevatedButton.styleFrom(backgroundColor: Colors.brown, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: loading ? const CircularProgressIndicator(color: Colors.white) : const Text("Save Changes", style: TextStyle(fontSize: 16, color: Colors.white, fontWeight: FontWeight.bold)))),
                ],
              ),
            ),
          ],
